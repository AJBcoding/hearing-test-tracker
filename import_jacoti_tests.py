#!/usr/bin/env python3
"""
Import script for Jacoti audiogram images using OCR.
...
def insert_jacoti_test(conn, ocr_result, image_path):
    """Insert a Jacoti hearing test with OCR-extracted measurements."""
    cursor = conn.cursor()
    ...
    """, (
        ...
        f'Imported from Jacoti OCR. Raw footer: {metadata.get("raw_footer_text", "")}',
        str(image_path),
        confidence
    ))
...
def main():
    """Import all Jacoti audiogram images from the archive."""
    ...
    for image_path in image_files:
        try:
            ...
            ocr_result = parse_jacoti_audiogram(image_path)
